...
python -c "from pymongo import MongoClient; import os; from dotenv import load_dotenv; load_dotenv('backend/.env'); client = MongoClient(os.getenv('MONGO_URI', 'mongodb://localhost:27017/')); db = client[os.getenv('MONGO_DB_NAME', 'chathub')]; user = db.users.find_one(); has_admin = 'is_admin' in user if user else False; print('OK: Database schema updated' if has_admin else 'WARNING: Need to run migration'); exit(0 if has_admin else 1)" 2>nul
if %ERRORLEVEL% NEQ 0 (
