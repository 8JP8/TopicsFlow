...
const Step4AuthenticatorVerify: React.FC<Step4Props> = ({ data, updateData, onNext, onBack }) => {
  const { t } = useLanguage();
  ...
  const handleVerify = async (code: string) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        // Save backup codes to registration data
        if (result.backup_codes) {
          updateData({ backupCodes: result.backup_codes });
        }
